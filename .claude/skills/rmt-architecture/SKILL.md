...
- **`ActivityFeedService`** — `snapshot()`: retorna feed público de atividade + contador de escrow. Busca pedidos `OrderStatus::Completed` via Eloquent (nunca expõe comprador — apenas `seller_name`). **Cache Redis 15s** (`Cache::remember('activity.snapshot', 15, fn() => ...)`); **Octane-safe**: sem estado em propriedade, lê conexão a cada chamada. `in_escrow_count` = pedidos `AwaitingConfirmation`. Rota servida por `Marketplace\ActivityController` (thin) + `Marketplace\ActivityItemResource`.
